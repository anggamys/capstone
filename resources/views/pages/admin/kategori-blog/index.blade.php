<x-app-layout>
    ...
    <div class="py-2" x-data="{ deleteModalOpen: false, deleteActionUrl: '' }">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-[#2B3674]">Data Kategori Blog</h1>
            <p class="text-sm text-slate-400 mt-1 font-medium">Kelola seluruh data kategori blog secara komprehensif.</p>
        </div>

        <!-- Reusable Search Component -->
        <x-admin-search 
            placeholder="Cari kategori blog.." 
            :action="route('admin.kategori-blog.create')" 
                ...
                <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                    ...
                    <td class="px-8 py-5 text-sm w-44">
                        <div class="flex items-center gap-4 text-[#89A8E0]">
                            ...
                            <a href="{{ route('admin.kategori-blog.show', $category->id) }}" class="hover:text-[#3F5C7D] transition-colors duration-150" title="Detail">
                                <x-lucide-eye class="w-5 h-5" />
                            </a>
                            
                            <!-- Edit (Orange Pencil Icon) -->
                            <a href="{{ route('admin.kategori-blog.edit', $category->id) }}" class="hover:text-amber-600 transition-colors duration-150 text-amber-500" title="Edit">
                                <x-lucide-pencil class="w-5 h-5" />
                            </a>

                            <!-- Delete (Red Trash Icon) -->
                            <button type="button" @click="deleteModalOpen = true; deleteActionUrl = '{{ route('admin.kategori-blog.destroy', $category->id) }}'" class="hover:text-rose-600 transition-colors duration-150 text-rose-500 align-middle" title="Hapus">
        ...
        <x-delete-modal 
            ...
            message="Apakah Anda yakin ingin menghapus kategori blog ini? Tindakan ini tidak dapat dibatalkan." 
        />
    </div>
</x-app-layout>
