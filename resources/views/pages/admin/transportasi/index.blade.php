<x-app-layout>
    ...
    <div class="py-2" x-data="{ deleteModalOpen: false, deleteActionUrl: '' }">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-[#2B3674]">Data Transportasi</h1>
            <p class="text-sm text-slate-400 mt-1 font-medium">Kelola seluruh data transportasi secara komprehensif.</p>
        </div>

        <!-- Reusable Search Component -->
        <x-admin-search 
            placeholder="Cari transportasi.." 
            :action="route('admin.transportasi.create')" 
            buttonText="Tambah Data" 
        />

        <!-- Reusable Table Component -->
        <x-admin-tabel :headers="['No', 'Nama Transportasi', 'Slug', 'Status', 'Aksi']" :items="$transportations">
            @forelse($transportations as $index => $transportation)
                <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                    <!-- Column 1: No -->
                    <td class="px-8 py-5 text-sm font-semibold text-[#2B3674] w-20">
                        {{ $transportations->firstItem() + $index }}
                    </td>
                    <!-- Column 2: Nama Transportasi -->
                    ...
                    <td class="px-8 py-5 text-sm w-44">
                        <div class="flex items-center gap-4 text-[#89A8E0]">
                            <!-- Detail View -->
                            <a href="{{ route('admin.transportasi.show', $transportation->id) }}" class="hover:text-[#3F5C7D] transition-colors duration-150" title="Detail">
                            ...
                            <!-- Edit -->
                            <a href="{{ route('admin.transportasi.edit', $transportation->id) }}" class="hover:text-amber-600 transition-colors duration-150 text-amber-500" title="Edit">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
                            ...
                            <!-- Delete -->
                            <button type="button" @click="deleteModalOpen = true; deleteActionUrl = '{{ route('admin.transportasi.destroy', $transportation->id) }}'" class="hover:text-rose-600 transition-colors duration-150 text-rose-500 align-middle" title="Hapus">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" class="w-5 h-5">
        ...
        <x-delete-modal 
            ...
            message="Apakah Anda yakin ingin menghapus transportasi ini? Tindakan ini tidak dapat dibatalkan." 
        />
    </div>
</x-app-layout>
