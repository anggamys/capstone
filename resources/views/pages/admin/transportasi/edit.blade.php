<x-app-layout>
    <x-slot name="header">
        {{ __('Transportasi | Edit') }}
    </x-slot>

    <div class="py-2">
        <form action="{{ route('admin.transportasi.update', $transportation->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div>
                    <h1 class="text-2xl font-bold text-[#2B3674]">Edit Transportasi</h1>
                    <p class="text-sm text-slate-400 mt-1 font-medium">Ubah informasi untuk transportasi tersebut</p>
                </div>
                <div>
                    <a href="{{ route('admin.transportasi.index') }}" class="inline-flex items-center justify-center px-6 py-3 border border-[#3F5C7D]/30 text-[#3F5C7D] hover:bg-[#3F5C7D]/5 text-sm font-bold rounded-2xl transition-all duration-200 shadow-sm">
                        Batal
                    </a>
                </div>
            </div>
            ...
            <div class="bg-white rounded-3xl p-8 shadow-sm border border-indigo-100/10 mb-6">
                ...
                <div class="flex items-center gap-3 mb-8 border-b border-slate-100 pb-4">
                    ...
                    <h2 class="text-base font-bold text-[#2B3674]">Transportasi</h2>
                </div>

                <div class="space-y-6">
                    <!-- Input 1: Nama Transportasi -->
                    <div>
                        <label for="name" class="block text-sm font-bold text-[#2B3674] mb-2">Nama Transportasi</label>
                        <input type="text" 
                               name="name" 
                               id="name" 
                    ...
                    <!-- Input 2: Slug -->
                    <div>
                        <label for="slug" class="block text-sm font-bold text-[#2B3674] mb-2">Slug Transportasi</label>
                        <input type="text" 
                               name="slug" 
                               id="slug" 
                               value="{{ old('slug', $transportation->slug) }}" 
                               placeholder="Contoh: sepeda-motor" 
                    ...
                    <div>
                        ...
                        <x-select-input 
                            name="status"
                            :value="old('status', $transportation->status)"
