<x-app-layout>
    ...
    <div class="py-2" x-data="{ deleteModalOpen: false, deleteActionUrl: '' }">
        ...
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-[#2B3674]">Data Aktivitas Wisata</h1>
            <p class="text-sm text-slate-400 mt-1 font-medium">Kelola seluruh data aktivitas wisata secara komprehensif.</p>
        </div>

        <!-- Reusable Search Component -->
        <x-admin-search 
            placeholder="Cari aktivitas.." 
            :action="route('admin.aktivitas.create')" 
            buttonText="Tambah Data" 
        />

        <!-- Reusable Table Component -->
        <x-admin-tabel :headers="['No', 'Nama Aktivitas', 'Slug', 'Status', 'Aksi']" :items="$activities">
                ...
                <tr class="hover:bg-slate-50/50 transition-colors duration-150">
                    ...
                    <td class="px-8 py-5 text-sm w-44">
                        <div class="flex items-center gap-4 text-[#89A8E0]">
                            <!-- Detail View -->
                            <a href="{{ route('admin.aktivitas.show', $activity->id) }}" class="hover:text-[#3F5C7D] transition-colors duration-150" title="Detail">
                                <x-lucide-eye class="w-5 h-5" />
                            </a>
                            
                            <!-- Edit -->
                            <a href="{{ route('admin.aktivitas.edit', $activity->id) }}" class="hover:text-amber-600 transition-colors duration-150 text-amber-500" title="Edit">
                                <x-lucide-pencil class="w-5 h-5" />
                            </a>
 
                            <!-- Delete -->
                            <button type="button" @click="deleteModalOpen = true; deleteActionUrl = '{{ route('admin.aktivitas.destroy', $activity->id) }}'" class="hover:text-rose-600 transition-colors duration-150 text-rose-500 align-middle" title="Hapus">
            ...
            @empty
                <tr>
                    <td colspan="5" class="px-8 py-10 text-center text-slate-400 font-medium">
                        Tidak ada data aktivitas wisata.
                    </td>
                </tr>
            @endforelse
        </x-admin-tabel>
        ...
        <x-delete-modal 
            ...
            message="Apakah Anda yakin ingin menghapus aktivitas wisata ini? Tindakan ini tidak dapat dibatalkan." 
        />
    </div>
</x-app-layout>
