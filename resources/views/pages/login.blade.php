        ...
        <form method="POST" action="{{ route('login') }}" class="w-full">
            ...
            <div class="mb-4">
                <input id="email" type="email" name="email" value="{{ old('email') }}" required autofocus autocomplete="username" placeholder="Email"
                       class="w-full px-5 py-4 rounded-2xl bg-white border-0 text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-sky-500/50 text-base shadow-inner transition-all duration-200" />
                @error('email')
            ...
            <!-- Submit Button -->
            <button type="submit" 
                    class="w-full py-4 rounded-2xl bg-[#345275] hover:bg-[#2d4766] text-white font-semibold text-base transition-all duration-200 shadow-md hover:shadow-lg active:translate-y-[1px]">
                Log In
            </button>
        </form>
